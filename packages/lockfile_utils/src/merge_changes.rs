...
use crate::{
    comver_to_semver,
    types::{Lockfile, ProjectSnapshot},
};
use semver::Version;
...
pub fn merge_changes(ours: Lockfile, theirs: Lockfile) -> Lockfile {
    let theirs_semver =
    ...
    let mut new_lockfile = Lockfile::new(if theirs_semver > ours_semver {
